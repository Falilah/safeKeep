...
library LibDMS {
    ...
    function getCurrentAllocatedEth() internal view returns (uint256) {
        ...
        for (uint256 x; x < dmsData.inheritors.length; x++) {
            totalEthAllocated += dmsData.inheritorWeishares[
                dmsData.inheritors[x]
            ];
        }
    ...
    function getCurrentAllocatedTokens(
        ...
        for (uint256 x; x < dmsData.inheritors.length; x++) {
            totalTokensAllocated += dmsData.inheritorTokenShares[
                dmsData.inheritors[x]
            ][_token];
        }
    ...
    function getCurrentAllocated1155tokens(
        ...
        DMSData storage dmsData = LibStorageBinder._bindAndReturnDMSStorage();
        for (uint256 x; x < dmsData.inheritors.length; x++) {
            alloc_ += dmsData.inheritorERC1155TokenAllocations[
                dmsData.inheritors[x]
            ][_token][_tokenID];
        }
    }

    function _isERC721Allocated(
        address _token,
        uint256 _tokenId
    ) internal view returns (bool allocated_) {
        DMSData storage dmsData = LibStorageBinder._bindAndReturnDMSStorage();
    ...
    function _resetClaimed(address _inheritor) internal {
        ...
        if (
            dmsData.inheritorAllocatedERC721TokenAddresses[_inheritor].length >
            0
        ) {
        ...
        if (
            dmsData.inheritorAllocatedERC1155TokenAddresses[_inheritor].length >
            0
        ) {
    ...
    function reset(address _inheritor) internal {
        ...
        if (dmsData.inheritorAllocatedERC20Tokens[_inheritor].length > 0) {
            for (
                uint256 x;
                x < dmsData.inheritorAllocatedERC20Tokens[_inheritor].length;
                x++
            ) {
                dmsData.inheritorTokenShares[_inheritor][
                    dmsData.inheritorAllocatedERC20Tokens[_inheritor][x]
                ] = 0;
                dmsData.inheritorActiveTokens[_inheritor][
                    dmsData.inheritorAllocatedERC20Tokens[_inheritor][x]
                ] = false;
            }
            //remove all token addresses
        ...
        if (
            ...
            for (
                ...
                address tokenAddress = dmsData
                    .inheritorAllocatedERC721TokenAddresses[_inheritor][x];
                uint256 tokenAllocated = dmsData.inheritorERC721Tokens[
                    _inheritor
                ][tokenAddress];
                if (tokenAllocated == 0) {
                ...
                dmsData.allocatedERC721Tokens[tokenAddress][
                    tokenAllocated
                ] = false;
                //also reset reverse allocation mapping
                dmsData.ERC721ToInheritor[tokenAddress][
                    tokenAllocated
                ] = address(0);
                delete dmsData.inheritorAllocatedTokenIds[_inheritor][
                    tokenAddress
                ];
            }
            //remove all token addresses
        ...
        if (
            ...
            for (
                ...
                dmsData.inheritorERC1155TokenAllocations[_inheritor][
                    dmsData.inheritorAllocatedERC1155TokenAddresses[_inheritor][
                        x
                    ]
                ][
                        dmsData.inheritorAllocatedTokenIds[_inheritor][
                            dmsData.inheritorAllocatedERC1155TokenAddresses[
                                _inheritor
                            ][x]
                        ][x]
                    ] = 0;
    ...
    function _allocateERC20Tokens(
        ...
        for (uint256 k; k < _inheritors.length; k++) {
            ...
            if (
                !dmsData.inheritorActiveTokens[_inheritors[k]][token] &&
                _shares[k] > 0
            ) {
                dmsData.inheritorAllocatedERC20Tokens[_inheritors[k]].push(
                    token
                );
            ...
            if (_shares[k] == 0) {
                LibArrayHelpers.removeAddress(
                    dmsData.inheritorAllocatedERC20Tokens[_inheritors[k]],
                    token
                );
                //double-checking
    ...
    function _allocateERC721Tokens(
        ...
        for (uint256 k; k < _inheritors.length; k++) {
            ...
            //short-circuit
            if (
                dmsData.ERC721ToInheritor[_token][_tokenIDs[k]] ==
                _inheritors[k]
            ) {
                continue;
            }
            ...
            try IERC721(_token).ownerOf(_tokenIDs[k]) returns (address owner) {
                if (owner == address(this)) {
                    if (dmsData.allocatedERC721Tokens[_token][_tokenIDs[k]]) {
                        address current = dmsData.ERC721ToInheritor[_token][
                            _tokenIDs[k]
                        ];
                        ...
                        if (
                            ...
                            LibArrayHelpers.removeUint(
                                dmsData.inheritorAllocatedTokenIds[current][
                                    _token
                                ],
                                _tokenIDs[k]
                            );
                        ...
                        if (_inheritors[k] == address(0)) {
                            dmsData.allocatedERC721Tokens[_token][
                                _tokenIDs[k]
                            ] = false;
                            LibArrayHelpers.removeUint(
                                dmsData.inheritorAllocatedTokenIds[current][
                    ...
                    dmsData.ERC721ToInheritor[_token][
                        _tokenIDs[k]
                    ] = _inheritors[k];
                    if (
                        dmsData
    ...
    function _allocateERC1155Tokens(
        ...
        for (uint256 i; i < _inheritors.length; i++) {
            ...
            dmsData.inheritorERC1155TokenAllocations[_inheritors[i]][_token][
                _tokenIDs[i]
            ] = _amounts[i];
            //if id is just being added
            if (
                !LibArrayHelpers._inUintArray(
                    dmsData.inheritorAllocatedTokenIds[_inheritors[i]][_token],
                    _tokenIDs[i]
                )
            ) {
                dmsData.inheritorAllocatedTokenIds[_inheritors[i]][_token].push(
                    _tokenIDs[i]
                );
            }
    ...
    function _claimERC20Tokens() internal {
        DMSData storage dmsData = LibStorageBinder._bindAndReturnDMSStorage();
        uint256 tokens = dmsData
            .inheritorAllocatedERC20Tokens[msg.sender]
            .length;
        if (tokens > 0) {
            for (uint256 i; i < tokens; i++) {
                address token = dmsData.inheritorAllocatedERC20Tokens[
                    msg.sender
                ][i];
                if (token == address(0)) {
                    continue;
                }
                uint256 amountToClaim = dmsData.inheritorTokenShares[
                    msg.sender
                ][token];
                if (amountToClaim > 0) {
                    //reset storage
    ...
    function _claimERC721Tokens() internal {
        ...
        if (tokens > 0) {
            for (uint256 i; i < tokens; i++) {
                address token = dmsData.inheritorAllocatedERC721TokenAddresses[
                    msg.sender
                ][i];
                if (token == address(0)) {
                    continue;
                }
                uint256 tokensToClaim = dmsData
                .inheritorAllocatedTokenIds[msg.sender][token].length;
                if (tokensToClaim > 0) {
                    for (uint256 j; j < tokensToClaim; j++) {
                        uint256 tokenID = dmsData.inheritorAllocatedTokenIds[
                            msg.sender
                        ][token][j];
                        if (tokenID == 0) {
                            //check for whitelist
                            ...
                            dmsData.inheritorAllocatedTokenIds[msg.sender][
                                token
                            ][j] = 0;
                            IERC721(token).transferFrom(
                                address(this),
    ...
    function _claimERC1155Tokens() internal {
        ...
        if (tokens > 0) {
            for (uint256 i; i < tokens; i++) {
                address token = dmsData.inheritorAllocatedERC1155TokenAddresses[
                    msg.sender
                ][i];
                if (token == address(0)) {
                    continue;
                }
                uint256 noOfTokenIds = dmsData
                .inheritorAllocatedTokenIds[msg.sender][token].length;
                if (noOfTokenIds > 0) {
                    for (uint256 k; k < noOfTokenIds; k++) {
                        ...
                        uint256 amount = dmsData
                            .inheritorERC1155TokenAllocations[msg.sender][
                                token
                            ][tokenID];
                        if (amount > 0) {
                            dmsData.inheritorERC1155TokenAllocations[
                                msg.sender
                            ][token][tokenID] = 0;
